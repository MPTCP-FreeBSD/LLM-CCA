...
files = ['1 flow latency.csv', '2 flow latency.csv', '4 flow latency.csv']
...
# 确保 'Latency' 列为数值类型
combined_df['prague-latency'] = pd.to_numeric(combined_df['prague-latency'], errors='coerce')
combined_df['cubic-latency'] = pd.to_numeric(combined_df['cubic-latency'], errors='coerce')

# 检查是否有非数值数据
non_numeric_rows = combined_df[combined_df[['prague-latency', 'cubic-latency']].isna().any(axis=1)]
print(non_numeric_rows)

# 删除包含非数值 'Latency' 值的行
combined_df = combined_df.dropna(subset=['prague-latency', 'cubic-latency'])

# 重塑数据以便于绘图
combined_df_melted = combined_df.melt(id_vars=['Interval start', 'Condition'], 
                                      value_vars=['prague-latency', 'cubic-latency'],
                                      var_name='Algorithm', value_name='Latency')
...
sns.boxplot(x='Condition', y='Latency', hue='Algorithm', data=combined_df_melted, showfliers=False, hue_order=['cubic-latency', 'prague-latency'])
plt.title('Latency Comparison Under Different Conditions')
