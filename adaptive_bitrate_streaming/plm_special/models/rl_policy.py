...
#             state_feature_dim,
#             bitrate_levels,
#             state_encoder,  # 你的 state_encoder 处理 [RTT, CWND, Throughput]
#             plm,
#             plm_embed_size,
#             max_length=None,
#             max_ep_len=100,
...
#         self.embed_state1 = nn.Linear(state_feature_dim, plm_embed_size).to(device)  # 处理经过 state_encoder 的状态

#         self.embed_ln = nn.LayerNorm(plm_embed_size).to(device)
#         # =========== multimodal encoder (end) ===========
    
#         self.action_head = nn.Linear(plm_embed_size, bitrate_levels).to(device)  # 输出动作（cwnd）
...
#         前向传播函数，用于训练。
#         """
#         assert actions.shape[0] == 1, 'batch size 应为 1，以避免 CUDA 内存不足'

#         # Step 1: 处理动作、returns 和 timesteps
...
#         # Step 2: 处理状态并将其转化为嵌入
#         states = states.to(self.device)  # shape: (1, seq_len, 3) 包含 [rtt, cwnd, throughput]
#         states_features = self.state_encoder(states)
#         states_embeddings1 = self.embed_state1(states_features) + time_embeddings

#         # Step 3: 拼接 returns, states, actions 嵌入
#         stacked_inputs = []
#         action_embed_positions = np.zeros(returns_embeddings.shape[1])  # 记录动作嵌入的位置
...
#         stacked_inputs = stacked_inputs[:, -self.plm_embed_size:, :]  # 截断序列长度，不超过 plm_embed_size
