...
class Trainer:
    ...
    def train_step(self, batch):
        ...
        # 前向传播，获取预测的 actions_pred
        actions_pred = self.model(states, actions, returns, timesteps)
        
        # 调整 actions_pred 的形状，使其与 labels 形状一致
        actions_pred = actions_pred.squeeze(-1)  # 将形状从 (1, 20, 1) 调整为 (1, 20)
        
        labels = labels.float()  # 确保 labels 是 float 类型
